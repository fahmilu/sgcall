(function($) {
    $(function() {
        ...
        //  * Form submit events
        //  */
        // $('form#testimonial, form#categories').live('submit', function(e) {
        //     e.preventDefault();
            ...
            $('.testi-list tbody').sortable({
                ...
                stop: function(event, ui) {
                    ...
                    $('td.action-to input').each(function(index) {
                        ...
                        order[index] = testimonial_id;
                    });
                    var csrfName = '<?php echo $this->security->get_csrf_token_name(); ?>',
                    csrfHash = '<?php echo $this->security->get_csrf_hash(); ?>';
                    $.post(SITE_URL + 'admin/testimonials/update_order', { csrfName : csrfHash,  order : order }, function(data, response, xhr) {
        ...
        {
            var url = SITE_URL + 'admin/testimonial';
            if(form_id == 'categories')
            {
                url = SITE_URL + 'admin/testimonial/categories';
            }
            $.ajax({
                url: post_url,
                type: "POST",
