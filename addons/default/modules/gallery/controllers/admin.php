...
class Admin extends Admin_Controller
{
	...
	private $validation_rules = array(
		array(
			'field' => 'userfile',
			'label' => 'lang:gallery_upload_label',
			'rules' => 'trim'
		),
		// array(
		// 	'field' => 'title',
		// 	'label' => 'lang:gallery_title_label',
		// 	'rules' => 'trim'
		// ),
		// array(
		// 	'field' => 'body',
		// 	'label' => 'lang:gallery_caption_label',
		// 	'rules' => 'trim'
		// ),
		array(
			'field' => 'curfile',
		...
		array(
			'field' => 'url',
			'label' => 'lang:gallery_url_label',
			'rules' => 'trim'
		),
		// array(
		// 	'field' => 'url_name',
		// 	'label' => 'lang:gallery_url_name',
		// 	'rules' => 'trim'
		// ),
		// array(
		// 	'field' => 'credit',
		// 	'label' => 'lang:gallery_credit_label',
		// 	'rules' => 'trim'
		// ),
		// array(
		// 	'field' => 'description',
		// 	'label' => 'lang:gallery_description_label',
		// 	'rules' => 'trim'
		// ),
		array(
			'field' => 'category_id',
	...
	public function __construct()
	{
		...
		$this->load->model(array('gallery_m', 'gallery_categories_m'));
		$this->load->library('form_validation');
		$this->lang->load('gallery');
		$this->load->helper(array('html'));

		$this->upload_cfg['upload_path']		= UPLOAD_PATH . 'gallery';
		...
		if ($this->settings->gallery_height)
			$this->max_height = $this->settings->gallery_height;

		if ($this->settings->gallery_width)
			$this->max_width = $this->settings->gallery_width;

		$this->_path = $this->upload_cfg['upload_path'].'/';
		$this->check_dir($this->_path);

		$no_category[''] = lang('gallery_category_option');

		$categories = $this->gallery_categories_m->category_options();
	...
	public function index($id=0)
	{
		if(isset($_SERVER['HTTP_REFERER'])){
			if(!preg_match("/gallery/",$_SERVER['HTTP_REFERER'])){
				$this->session->unset_userdata('category');
			}
		}

		if ($this->session->userdata('category')) {
			$slides = $this->gallery_m->get_all_by_category($this->session->userdata('category'));
			
		} else {
			$slides = $this->gallery_m->get_all();
		}

		// $slides = $this->gallery_m->get_all($id);
		// print_r($slides); die();
		$this->template
	...
	public function upload()
	{
		...
		if ($this->form_validation->run())
		{
			...
			else
			{
				...
				// Insert success
				if ($id = $this->gallery_m->insertimg($data))
				{
					$status		= 'success';
					$message	= lang('gallery_upload_img_success');
				}
				// Insert error
				else
				{
					...
					$status		= 'error';
					$message	= sprintf(lang('gallery_dbstore_img_error'), $file['file_name']);
				}
				...
				if ($this->input->is_ajax_request())
				{
					...
					return $this->template->build_json(array(
						...
						'file'		=> array(
							...
							'type'	=> $file['file_type'],
							'size'	=> $file['file_size'],
							'thumb'	=> UPLOAD_PATH . 'gallery/' . $thumbfile
						)
					));
				}
				else
				{
					...
					($this->input->post('btnAction') == 'save_exit') ? redirect('admin/gallery') : redirect('admin/gallery/edit/'.$id);
				}
	...
	{
		if (!$id OR !$slide=$this->gallery_m->get($id))
		{
			$this->session->set_flashdata('error', lang('gallery_edit_no_id_error'));
    ...
    //             'url_name'		    => $this->input->post('url_name'),
			...
			// Insert success
			if ($this->gallery_m->update($id, $data))
			{
				$status		= 'success';
				$message	= lang('gallery_upload_img_success');
			}
			...
			$this->session->set_flashdata($status, $message);
			($this->input->post('btnAction') == 'save_exit') ? redirect('admin/gallery') : redirect('admin/gallery/edit/'.$id);//redirect('admin/gallery');
	...
	{
		$data->files = $this->gallery_m->get_all($id);
	...
	public function delete($id=0)
	{
		...
		{
			$status = 'error';
			$message = lang('gallery_delete_no_id_error');
			$data = array();
			$data['messages'][$status] = $message;
			$message = $this->load->view('admin/partials/notices', $data, TRUE);
		...
		$slide = $this->gallery_m->get($imgid);

		if ($this->gallery_m->delete($imgid))
		{
		...
		{
			$status = 'error';
			$message = lang('gallery_delete_error');
		}
	...
	public function unlinkfile($filename)
	{
			...
			$stat = array();
					...
					if (file_exists($this->_path .'/' . $fullfile))
					{
						...
						{
							$stat[] = 'error';
							$msg[] = sprintf(lang('gallery_delete_img_error'), $fullfile);
						}
					}
					...
					if (file_exists($this->_path . '/' . $thumbfile))
					{
						...
						{
							$stat[] = 'error';
							$msg[] = sprintf(lang('gallery_delete_img_error'), $thumbfile);
						}
					}
					...
					if (file_exists($this->_path . '/' . $filename))
					{
						...
						{
							$stat[] = 'error';
							$msg[] = sprintf(lang('gallery_delete_img_error'), $filename);
						}
					}
					...
					{
						$status = 'success';
						$message = sprintf(lang('gallery_delete_img_success'), $filename);
					}
	...
	public function imgorder()
	{
		...
		foreach ($ids as $id)
		{
			$this->gallery_m->updateimg($id, array(
				'position' => $i
			));
			++$i;
		}
	...
	function _check_ext()
	{
		if ( ! empty($_FILES['userfile']['name']))
		{
			...
			if ( ! $this->_ext)
			{
				$this->form_validation->set_message('_check_ext', lang('gallery_invalid_extension'));
				return FALSE;
			}
		}
		elseif ($this->method === 'upload')
		{
			...
			#	return TRUE;
//echo "returning false! ".lang('gallery_upload_error')."<br />";
			$this->form_validation->set_message('_check_ext', lang('gallery_upload_error'));
			return FALSE;
		}
