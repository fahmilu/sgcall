...
<section class="title">
	<h4><?php echo lang('banner_title'); ?></h4>
</section>
...
<div class="content">
	<fieldset id="filters">
        ...
        <?php echo form_open('admin/banner/filterbycategory', array('method' => 'post')); ?>
            <ul>
                <li>
	...
	<ul id="files-uploaded" class="grid clear-both">
	    ...
	    <?php foreach($slides as $file): ?>
	        <li class="ui-corner-all">
	            ...
	            <div style="background: url(<?php echo base_url(UPLOAD_PATH.$path. substr($file->filename, 0, -4) . substr($file->filename, -4));?>) no-repeat center center; background-size: cover; padding-bottom: 65%; margin: 0px 10px;"></div></a>
				<div class="actions">
					<?php
						// echo anchor('admin/banner/crop/'.$file->id, lang('banner_crop_label'), 'class="imgcrop"' ) . ' | ';
						//echo anchor('admin/banner/editmedia/'.$file->id, lang('buttons.edit'), 'class="imgedit"' );					
						if (group_has_role('banner', 'edit_image')) {
							echo anchor('admin/banner/edit/'.$file->id, lang('buttons:edit'), 'class="imgedit"' ) . '|';
						}
						if (group_has_role('banner', 'delete_image')) {
							echo anchor('admin/banner/delete', lang('buttons:delete'), 'class="imgdel"' );
						}
	...
	<?php else: ?>
		...
		<div class="no_data"><?php echo lang('banner_no_slides_error'); ?></div>
	...
	<div style="font-size:0.9em; padding-top:9px;"><?php echo lang('banner_dragndrop_msg'); ?></div>
			...
			<div class="hidden">
				<div id="files-uploader">
					<div class="files-uploader-browser">
						<form action="<?php echo site_url('/admin/banner/upload') ?>" method="post" accept-charset="utf-8" enctype="multipart/form-data" name="frmUpload">
							...
							<?php if (!empty($gallery->id)): ?>
								<input type="hidden" id="banner_id" name="banner_id" value="<?php echo $gallery->id; ?>" />
							<?php endif; ?>
						</form>
...
<script type="text/javascript">
	...
	var startbtn = '<?php echo lang('banner_start_label'); ?>';
	var cancelbtn = '<?php echo lang('banner_delete_label'); ?>';

	var noimg = '<div class="no_data"><?php echo lang('banner_no_slides_error'); ?></div>';
