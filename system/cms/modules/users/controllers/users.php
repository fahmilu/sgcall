...
class Users extends Public_Controller
{
	...
	public function login()
	{
		...
		if ($this->form_validation->run() or $this->current_user)
		{
			...
			{
				redirect($redirect_to ? $redirect_to : 'profile');
			}
		}
	...
	public function register()
	{
		...
		if ($_POST)
		{
			if ($this->form_validation->run())
			{
			    ...
			    $profile_data['last_name'] = (strpos($name, ' ') === false) ? '' : preg_replace('#.*\s([\w-]*)$#', '$1', $name);
			    $profile_data['first_name'] = trim( preg_replace('#'.$profile_data['last_name'].'#', '', $name ) );
				...
				$this->sobat->createUserPoint($id, 12);
				$this->sobat->insertQuestPoint($id, 0, "", "", 12, $status=1);
