...
class Plugin_Contact extends Plugin 
{
	...
	public function form()
	{
		...
		$this->form_validation->set_error_delimiters('<div class="alert alert-danger">X - ', '</div>');
		...
		$redirect           = $this->attribute('success-redirect', 'contact-success');
		...
		foreach ($field_list as $field => $rules)
		{
			...
			switch ($rule_array[0]) {
				...
				case 'recaptcha':
					$form_meta[$field]['type'] = 'text';
					$other_rules = 'trim';
					if ($_POST && ! self::_check_recaptcha())
					{
			...
			if ($field=='recaptcha')
				$validation[$field]['rules'] = !empty($other_rules)?$other_rules:'trim';
			else
		...
		foreach ($form_meta as $form => $value)
		{
			...
			elseif ($value['type'] == 'textarea')
			{
				$opts = array(
					...
					'value'			=> set_value($form),
					'class'			=> $form=='message'?' notes ':'',
					'rows'			=> '3',
					'style'			=> '',
				);
				$parse_data[$form] .= call_user_func('form_'.$value['type'], $opts);

				//$parse_data[$form] .= call_user_func('form_'.$value['type'],
				//										$form,
				//										set_value($form),
				//										'id="contact_'.$form.'" placeholder="Enter your '.$form.'" class="'.$form.( $form=='message'?' notes ':'' ).'"'
				//									 );
			}
			elseif($form == 'recaptcha')
			{
				if ($this->settings->recaptcha_secretkey && $this->settings->recaptcha_publickey)
				{
					$parse_data[$form] .= '<div class="g-recaptcha" data-sitekey="'.$this->settings->recaptcha_publickey.'">&nbsp;</div>';
						//call_user_func('form_'.$value['type'],
						//								$form,
						//								$value['value'],
						//								'class="'.$form.'"'
						//							 );
...
*/
		...
		// $output .= '<div class="col-lg-4" style="padding-top:10px;"><button name="contact-submit" type="submit" value="submit">'.ucfirst($button).'</button></div>'.PHP_EOL;
		$output .= form_close();
	...
	{
		$recaptcha_end_point = 'https://www.google.com/recaptcha/api/siteverify';
		$secret = $this->settings->recaptcha_secretkey ? $this->settings->recaptcha_secretkey : '6LcerR8TAAAAALYXDra13oT9eyDYBWRG5Papq9Uo';
		$captcha_value = $this->input->post('g-recaptcha-response');
		$verification = $this->_fetchURL($recaptcha_end_point, array('secret'=>$secret, 'response'=>$captcha_value));

		$results = json_decode($verification);
...
/* */
		...
		else
		{
			if (!empty($results->{"error-codes"}))
				$msg = $this->captcha_errors[$results->{"error-codes"}[0]];
			else
				$msg = "Error processing Recaptcha. Please reactivate.";
			$ret_val = FALSE;
		}

		$this->form_validation->set_message('_check_recaptcha', $msg);
		return $ret_val;
	}

	private function _fetchURL($token_endpoint, $fields)
	{
		$fields_string = '';
