...
class Pages extends Public_Controller
{
	...
	public function _page($url_segments)
	{
		...
		// added by OS, 2018-05-04
		if (AUTO_LANGUAGE=='en')
		{
			if (!empty($page->body_en))
		...
		//
		// modified by OS 2015-10-24 to accommodate ID & EN versions
		//
		if ($_SESSION['lang_code']=='en')
		{
			$meta_title = ($page->meta_title_en ? $page->meta_title_en : $page->layout->meta_title);
			$meta_description = ($page->meta_description_en ? $page->meta_description_en : $page->layout->meta_description);
			$meta_keywords = '';
			if ($page->meta_keywords_en or $page->layout->meta_keywords)
			{
				$meta_keywords = $page->meta_keywords_en ?
									Keywords::get_string($page->meta_keywords_en) :
		...
		if ( ! $meta_title)
		{
			$meta_title = ($_SESSION['lang_code']=='en' ? $page->title_en : $page->title);
		}
