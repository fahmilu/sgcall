...
*/
		...
		// modified by OS: 2015-12-09
		$custom_streams = array();
		$c_streams = $this->streams->fields->get_stream_fields($stream->stream_slug, $stream->stream_namespace, $values);
		foreach($c_streams as $cs)
		{
			$custom_streams[$cs['field_slug']] = $cs;
		}
		...
		$this->template
			...
			->set('page', $page)
			->set('custom_streams', $custom_streams) // added by OS: 2015-12-09
	...
	public function edit($id = 0)
	{
		...
		// If there's a keywords_en hash
		if ($page->meta_keywords_en != '')
		{
			// Get comma-separated meta_keywords based on keywords hash
			$this->load->model('keywords/keyword_m');
			$old_keywords_en_hash = $page->meta_keywords_en;
			$page->meta_keywords_en = $this->keywords->get_string($page->meta_keywords_en);
		}
		...
		if ($this->form_validation->run())
		{
			...
			if (isset($old_keywords_en_hash))
			{
				$input['old_keywords_en_hash'] = $old_keywords_en_hash;
			}
		...
		{
			$custom_streams[$cs['field_slug']] = $cs;
			$custom_streams[$cs['field_slug']]['value'] = !empty($page->{$cs['field_slug']}) ? $page->{$cs['field_slug']} : NULL;
		}
