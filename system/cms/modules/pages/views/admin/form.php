				...
				<ul>
					...
					<li>
						...
						<div class="tabs">
							...
							<ul class="tab-menu">
								<li><a href="#content-id"><span>Bahasa Indonesia</span></a></li>
								<li><a href="#content-en"><span>English</span></a></li>
							</ul>
							...
							<div id="content-id">
								...
								<ul>
									...
									<?php echo $this->load->view('admin/partials/streams/form_single_display', array('field' => $stream_fields[0]), true) ?>
							...
							<div id="content-en">
								...
								<ul>
									...
									<?php echo $this->load->view('admin/partials/streams/form_single_display', array('field' => $custom_streams['title_en']), true) ?>									
									...
									<li>
										...
										<div class="input">
										<?php if ( ! empty($page->parent_id)): ?>
											<?php echo site_url($parent_page->uri_en?$parent_page->uri_en:$parent_page->uri) ?>/
										<?php else: ?>
											<?php echo site_url() . (config_item('index_page') ? '/' : '') ?>
										<?php endif ?>
					
										<?php if ($this->method == 'edit'): ?>
											<?php echo form_hidden('old_slug', $page->slug_en) ?>
										<?php endif ?>
					
										<?php if (in_array($page->slug_en, array('home', '404'))): ?>
											<?php echo form_hidden('slug_en', $page->slug_en) ?>
											<?php echo form_input('', $page->slug_en, 'id="slug_en" size="20" disabled="disabled"') ?>
										<?php else: ?>
											<?php echo form_input('slug_en', $page->slug_en, 'id="slug_en" size="20" class="'.($this->method == 'edit' ? ' disabled' : '').'"') ?>
										<?php endif ?>
									...
									<?php echo $this->load->view('admin/partials/streams/form_single_display', array('field' => $stream_fields[1]), true) ?>
					...
					<li>
						...
						<ul>
							<?php
								$our_streams = array();
								foreach ($stream_fields as $i => $field)
								{
									if ($field['field_slug'] == 'header_image')
									{
										echo $this->load->view('admin/partials/streams/form_single_display', array('field' => $field), true);
										//echo "<p>header_image found!</p>\n";
										//$our_streams[] = $stream_fields[$i];
									}
								}
/*
								foreach ($our_streams as $field)
								{
									//if ($field->field_slug <> 'header_image')
									//	continue;
									//else
						...
						<div id="content-id">
							<fieldset>
								...
								<ul>
									...
									<?php endif; ?>
									<li class="full_width_input">
										<label for="meta_description"><?php echo lang('pages:meta_desc_label') ?></label>
										<div class="input"><?php echo form_textarea(array('name' => 'meta_description', 'value' => $page->meta_description)) ?></div>
									</li>
								</ul>
						...
						<div id="content-en">
							<fieldset>
								...
								<ul>
									...
									<?php echo $this->load->view('admin/partials/streams/form_single_display', array('field' => $custom_streams['meta_title_en']), true) ?>									

									<?php if ( ! module_enabled('keywords')): ?>
										<?php echo form_hidden('meta_keywords_en'); ?>
									<?php else: ?>
										<?php echo $this->load->view('admin/partials/streams/form_single_display', array('field' => $custom_streams['meta_keywords_en']), true) ?>									
									<?php endif; ?>

									<?php echo $this->load->view('admin/partials/streams/form_single_display', array('field' => $custom_streams['meta_description_en']), true) ?>									
...
$(document).ready(function(){
	...
	if ($('#page-form').data('mode') == 'create') {
		pyro.generate_slug('input[name="title_en"]', 'input[name="slug_en"]');
	}
