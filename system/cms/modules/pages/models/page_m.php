...
class Page_m extends MY_Model
{
	...
	public $validate = array(
		...
		'slug_en' => array(
			'field' => 'slug_en',
			'label'	=> 'lang:global:slug',
			'rules'	=> 'trim|required|alpha_dot_dash|max_length[250]|callback__check_slug_en'
		),
		array(
			'field'	=> 'css',
	...
	public function get_by_uri($uri, $is_request = false, $simple_return = false)
	{
		...
		else
		{
			...
			while ( ! $page AND $uri AND $i < 15) /* max of 15 in case it all goes wrong (this shouldn't ever be used) */
			{
				...
				$page_reverse_lang = $this->pyrocache->model('page_m', 'get_page_simple_reverse_lang', array($uri));
				...
				if(!$page and $page_reverse_lang){
					
					if(AUTO_LANGUAGE == 'en'){
						redirect($page_reverse_lang->slug_en);
					}else{
						redirect($page_reverse_lang->slug);
					}
					break;
				}
	...
	{
		// modified by OS - 2015-10-22
		// join page_details
		//$this->db->join('page_details', 'page_id = pages.id AND lang_code = \'id\'', 'left');
		$all_pages = $this->db
			->select('id, parent_id, title, title_en, status')
			->order_by('`order`')
			->get('pages')
			->result_array();
		...
		foreach ($all_pages as $row)
		{
			$pages[$row['id']] = $row;
			if(AUTO_LANGUAGE == 'en'){
				if($pages[$row['id']]['title_en']){
					$pages[$row['id']]['title'] = $pages[$row['id']]['title_en'];
				}
			}
		}
	...
	public function get_page_simple_reverse_lang($uri)
	{
		if (AUTO_LANGUAGE=='en')
		{
			$this->db->where('uri', $uri);
	...
	public function get_descendant_ids($id, $id_array = array())
	{
		...
		$children = $this->db->select('id, title, title_en')
			->where('parent_id', $id)
			->get('pages')
			->result();
	...
	public function build_lookup($id)
	{
		...
		$segments = $segments_en = array();
		do
		{
			$page = $this->db
				->select('slug, slug_en, parent_id')
				->where('id', $current_id)
				->get('pages')
				->row();
			...
			array_unshift($segments_en, $page->slug_en);
		}
...
*/
		...
		return $this->update($id, array('uri' => implode('/', $segments), 'uri_en' => implode('/', $segments_en)), true);
	}
	...
	*/
	...
	{
		$this->validate['slug_en'] = array(
			'field' => 'slug_en',
			'label'	=> lang('global:slug').' (English)',
			'rules'	=> 'trim|alpha_dot_dash|max_length[250]|callback__check_slug_en['.$id.']'
		);
		...
		$result = $this->update($id, array(
			...
			'js'				=> isset($input['js']) ? $input['js'] : null,
			'meta_title'    	=> isset($input['meta_title']) ? $input['meta_title'] : '',
			'meta_title_en'    	=> isset($input['meta_title_en']) ? $input['meta_title_en'] : '',
			...
			'meta_keywords_en' 	=> isset($input['meta_keywords_en']) ? $this->keywords->process($input['meta_keywords_en'], (isset($input['old_keywords_en_hash'])) ? $input['old_keywords_en_hash'] : null) : '',
			'meta_robots_no_index'    => ! empty($input['meta_robots_no_index']),
			'meta_robots_no_follow'    => ! empty($input['meta_robots_no_follow']),	
			'meta_description' 	=> isset($input['meta_description']) ? $input['meta_description'] : '',
			'meta_description_en'=> isset($input['meta_description_en']) ? $input['meta_description_en'] : '',
			'rss_enabled'		=> ! empty($input['rss_enabled']),
			'comments_enabled'	=> ! empty($input['comments_enabled']),
			'status'			=> $input['status'],
			'updated_on'		=> now(),
...
//echo "<pre>\n"; echo "\$page_ids:\n"; print_r($page_ids); echo "<hr />\n";
	...
	*/
	...
	/**
	 * Check English Slug for Uniqueness
	 *
	 * English Slugs should be unique among sibling pages.
	 *
	 * @param string $slug The english slug to check for.
	 * @param int $parent_id The parent_id if any.
	 * @param int $id The id of the page.
	 *
	 * @return bool
	*/
	public function _unique_slug_en($slug, $parent_id, $id = 0)
	{		
		return (bool) parent::count_by(array(
			'id !=' => $id,
			'slug_en' => $slug,
	...
	// added by OS - slug english; 2018-04-27
	/**
	 * Callback to check uniqueness of slug + parent
	 *
	 * 
	 ...
	 * @return bool
	 */
	 public function _check_slug_en($slug)
	 {
	 	// This is only going to be set on Edit
	 	$page_id = $this->uri->segment(4);
	 	...
	 	// See if this slug exists already
		if ($this->_unique_slug_en($slug, $parent_id, (int) $page_id))
		{
			// Root Level, No Page
			if (empty($parent_id))
			{
			...
			$this->form_validation->set_message('_check_slug_en', sprintf(lang('pages:page_already_exist_error'), $url, $parent_folder));
			return false;
		}
	...
	{
		if (!empty($params['prev_next']))
		{
			$where = " (".$this->db->dbprefix('pages').".id < ".$params['prev_next']." OR ".$this->db->dbprefix('pages').".id > ".$params['prev_next'].") ";
			$this->db->where($where);
			$this->db->limit(2);
			//$this->db->join('page_types', 'page_types.id = pages.type_id', 'left');
			unset($params['prev_next']);
		}
