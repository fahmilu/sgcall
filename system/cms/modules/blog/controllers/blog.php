...
class Blog extends Public_Controller
{
	...
	public function view($slug = '')
	{
		...
		if($post['keywords']){
			$keywords = $this->blog_m->getkeywords($post['keywords']);

			$keywordsArray = array();
			foreach ($keywords as $key) {
				array_push($keywordsArray, $key['keyword']);
			}
			$getother = $this->blog_m->get_tagged_by_keywords($keywordsArray, $post['id']);
		}else{
			$getother = $this->blog_m->getother($post['id']);
		}
		// print_r($this->blog_m->get_tagged_by_keywords($keywordsArray, $post['id']));
		// echo $data['entries'][0]['id'];
		// exit();
		$next = $this->blog_m->getnext($post['id']);
		$prev = $this->blog_m->getprev($post['id']);
		$category = $this->blog_m->getcategory($post['category_id']);
		...
		$this->template
			->title($post['title'])
			->set_metadata('description', $post['body'])
			->set("post", $post)
			->set("next", $next)
			->set("prev", $prev)
			->set("getother", $getother)
