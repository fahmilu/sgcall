...
class Plugin_Blog extends Plugin
{
    ...
    public function count_pinned_article() {
        $dt =  
        $this->db->where('status', 'live')
	        ->where('category_id', 1)
	        ->where('pin_to_home', 'yes')
	...
	public function get_pinned_article() {
        $post = $this->db->select('blog.*, files.path as img_url')
        	->join('files', 'blog.banner_image = files.id', 'left')
          	->where('blog.status', 'live')
	        ->where('blog.category_id', 1)
	        ->where('blog.pin_to_home', 'yes')
	        ->limit((int) $this->attribute('limit', 3))
	        ->order_by($this->attribute('order_by', 'id', 'DESC'))
	        ->get('blog')
	        ->result();

        // foreach ($post as $testi) {
        //     $testi->url = base_url() . UPLOAD_PATH . 'post/'. $testi->photo;
        // }
        return $post;
    }
