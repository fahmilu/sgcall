...
class Blog_m extends MY_Model
{
	...
	public function getkeywords($hash){
		return $this->db->select('keywords.name as keyword')
		->join('keywords', 'keywords.id = keywords_applied.keyword_id')
		->where('keywords_applied.hash', $hash)
	...
	public function get_tagged_by_keywords($keywords, $id)
	{
		return $this->db->select('blog.*, blog.title title, blog.slug slug, blog_categories.title category_title, blog_categories.slug category_slug, profiles.display_name, files.path as banner_image')
			->from('blog')
			->join('keywords_applied', 'keywords_applied.hash = blog.keywords')
			->join('keywords', 'keywords.id = keywords_applied.keyword_id')
			->join('blog_categories', 'blog_categories.id = blog.category_id', 'left')
			->join('profiles', 'profiles.user_id = blog.author_id', 'left')
			->join('files', 'blog.banner_image = files.id', 'left')
			->where('blog.id !=', $id)
			->where_in('keywords.name', $keywords)
			->order_by('blog.id', 'random')
			->limit(3)
			->get();
	}
	...
	{
		return $this->db
			->select('blog.id, blog.slug, blog.title')
			->join('profiles', 'profiles.user_id = blog.author_id', 'left')
			->join('users', 'blog.author_id = users.id', 'left')
			->where('blog.id >', $id)
			->limit(1)
	...
	{
		return $this->db
			->select('blog.*, files.path as banner_image')
			->join('profiles', 'profiles.user_id = blog.author_id', 'left')
			->join('users', 'blog.author_id = users.id', 'left')
			->join('files', 'blog.banner_image = files.id', 'left')
			->where('blog.id !=', $id)
