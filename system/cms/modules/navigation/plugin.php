...
$uri_string = uri_string();
		...
		foreach ($links as $link)
		{
			...
			$item['url']   = $link['url'];
			$item['url_en']   = $link['url_en'];
			if ($_SESSION['lang_code']=='en')
			{
				$item['title'] = $link['title_en']?$link['title_en']:$link['title'];
				$item['uri'] = $link['uri_en']?$link['uri_en']:$link['uri'];
				$item['url'] = $link['url_en']?$item['url_en']:$item['url'];
			}
			else
			{
				$item['title'] = $link['title'];
			...
			$u_test = parse_url($link['url']);
			preg_match('/^[^\/]+/i', trim($u_test['path'], '/'), $match);
			$activate_current_menu = FALSE;
			if (!empty($url_segment) && !empty($match[0]) && $url_segment == $match[0])
			{
				$activate_current_menu = TRUE;
			}

			// is this the link to the page that we're on?
			if ( preg_match('@^' . current_url() . '/?$@', $link['url']) or $activate_current_menu or ($link['link_type'] == 'page' and $link['is_home']) and site_url() == current_url() )
			{
