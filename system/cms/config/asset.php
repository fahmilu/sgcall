...
$config['asset_min'] = (ENVIRONMENT === PYRO_PRODUCTION) && !(bool) preg_match('@\/admin(\/.+)?$@', $_SERVER['REQUEST_URI']);

/**
 * Whether to combine files
 */
////////$config['asset_combine'] = (ENVIRONMENT !== PYRO_DEVELOPMENT);
$config['asset_combine'] = (ENVIRONMENT === PYRO_PRODUCTION) && !(bool) preg_match('@\/admin(\/.+)?$@', $_SERVER['REQUEST_URI']);

/*
echo 'ENVIRONMENT: '.ENVIRONMENT.'<br />'.PHP_EOL;
var_dump((bool) !preg_match('@\/admin(\/.+)?$@', $_SERVER['REQUEST_URI']));
*/
